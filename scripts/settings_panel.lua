...
function settings_panel.create(player_index)
    ...
    flow.add { type = "label", caption = { np("line-gap") } }
    flow.add { type = "textfield", numeric = true, allow_decimal = true, text = g.line_gap and tostring(g.line_gap) or 0.08, name = "line_gap" }
...
local function save(player, frame)
    ...
    local line_gap_value = tonumber(field_table.line_gap.text)
    if not line_gap_value or line_gap_value < 0.08 or line_gap_value > 1 then
        player.print({ np("invalid_line_gap") })
        return
    end
